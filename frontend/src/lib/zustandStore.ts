  ...
  persist(
    (set, get) => ({
        ...
        try {
          ...
          if (response.ok) {
            ...
            if (userResponse.ok) {
              ...
              set({ user: userData });
              // Write auth cookie so Next.js proxy can detect login state
              document.cookie = `litrealm_auth=true; path=/; SameSite=Lax; max-age=86400`;
            }
      ...
      logout: () => {
        ...
        document.cookie = 'litrealm_auth=; path=/; max-age=0';
        set({
          user: null,
          token: null,
          library: [],
